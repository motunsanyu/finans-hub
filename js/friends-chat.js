...
const FriendsChatModule = (() => {
  ...
  async function loadConversations() {
    ...
    try {
      ...
      for (const f of friends) {
        const { data: msgs, error: msgErr } = await getSB()
          ...
          .limit(1);
        if (msgErr) console.warn('Mesaj sorgu hatası:', msgErr);
    ...
    } catch (e) { console.error('Konuşma listesi hatası:', e); el.innerHTML = '<p style="text-align:center; padding:20px; color:red;">Hata oluştu.</p>'; }
  }
  ...
  async function getMessageHistory(friendId) {
    ...
    try {
      const { data, error } = await getSB()
        .from('messages')
        .select('*')
        .filter('sender_id', 'in', `(${user.id},${friendId})`)
        .filter('receiver_id', 'in', `(${user.id},${friendId})`)
        .order('created_at', { ascending: true });
      if (error) throw error;
      return data || [];
    } catch (err) {
      console.error('getMessageHistory hatası:', err);
      return [];
    }
  }
  ...
  async function loadMessages() {
    ...
    try {
      ...
      const { data: { user } } = await getSB().auth.getUser();
      if (!messages.length) { list.innerHTML = '<p style="text-align:center; padding:40px 20px; color:var(--text-secondary);">Henüz mesaj yok.</p>'; return; }
      ...
      list.innerHTML = '<p style="text-align:center; padding:20px; color:red;">Mesajlar yüklenirken hata oluştu.</p>';
    }
  }
  ...
  window.deleteMessage = async function (scope) {
    ...
    try {
      ...
      if (scope === 'me') {
        ...
        removeMessageFromDOM(id);
        if (window.showToast) window.showToast('Mesaj herkesten silindi.', 'success');
      }
  ...
  window.deleteConversation = async function () {
    ...
    if (window.showToast) window.showToast('Sohbet gizlendi (yerel).', 'info');
  };
